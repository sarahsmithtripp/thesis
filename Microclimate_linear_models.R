...
#   ylab(paste('Adjusted Model R\u00b2')) + xlab("Radius (m)") + labs(color = "Model")  + scale_color_manual(values=c("#990000", "#cc0000", "#FF3333", "#6699CC"), labels =  c("Soil (C째)", "Surface (C째)", "Near Surface (C째)", "Soil Moisture (vol %)"))+
#   scale_shape_manual(values = c(15, 16, 17, 18), labels = c("Soil (C째)", "Surface (C째)", "Near Surface (C째)", "Soil Moisture (vol %)")) + 
...
variation_fixed <- ggplot(model_R_all, aes(Radius, pear_corr, color = Model, pch = Model)) + geom_point(size = 4) +
  ...
  scale_color_manual(values=c("#990000", "#cc0000", "#FF3333", "#6699CC"), labels =  c("Soil (C째)", "Surface (C째)", "Near Surface (C째)", "ln(Soil Moisture (vol %))"))+
  scale_shape_manual(values = c(15, 16, 17, 18), labels = c("Soil (C째)", "Surface (C째)", "Near Surface (C째)", "ln(Soil Moisture (vol %))")) +
  theme_bw(base_size = 20) 
...
facet_grid <- ggplot(climate_modeling_Ht_long %>% filter(Canopy_Radius %in% c("DAP_Canopy_Height_r2m", "DAP_Canopy_Height_r15m") & sensor == "T1"),
  ...
  geom_point() + 
  geom_smooth(method = "lm", alpha = 0.2, color = "grey") +
  ylab("Average Daily Max Temperature 째C") + 
  xlab("Average Canopy Height (m)") +
  facet_grid(Canopy_Radius ~ sensor) +   theme(axis.text.x =  element_text(margin = margin(r= 0.4, l = 0.4)))+
  ggthemes::scale_color_tableau(palette = "Classic Cyclic") +
  ggthemes::scale_fill_tableau(palette = "Classic Cyclic") + guides(fill = F) +theme_bw(base_size = 15)
...
annual_model_graph_range <- ggplot(avg_values_predictions_range) + 
  ...
  #geom_line(aes(DAP_Canopy_Height_r15m, upper_nr)) +
  xlim(0,25) +
  ylab("Mean Daily Range in Temperature 째C") + 
  labs(color = "Plot") +
  xlab("Canopy Height (m)")+
...
sm_annual_range_model_graph <- ggplot(soil_moist_ann_range_dat, aes(group = plot)) + 
  ...
  geom_line(aes(DAP_Canopy_Height_r2m, prediction, color = plot),size = 0.5, alpha = 0.5) +
  geom_line(aes(DAP_Canopy_Height_r2m, prediction_nr), color = "black", size = 1) +
...
save_plot(full_stack, filename = "D:/Data/SmithTripp/Gavin_Lake/Figures/annual_mods.jpeg", 
...
soil_coeff_df <- mods_coeff_function(climate_modeling_month, sensor = "T1", variable = "mean", class_name = "Soil 째C")
soil_coeff_df_range <- mods_coeff_function(climate_modeling_month, sensor = "T1", variable = "range", class_name = "Soil 째C")
...
surface_coeff_df <- mods_coeff_function(climate_modeling_month, sensor = "T2", variable = "mean", class_name = "Surface 째C")
surface_coeff_df_range <- mods_coeff_function(climate_modeling_month, sensor = "T2", variable = "range", class_name = "Surface 째C")
...
near_surface_coeff_df <- mods_coeff_function(climate_modeling_month, sensor = "T3", variable = "mean", class_name = "Near-Surface 째C")
near_surface_coeff_df_range <- mods_coeff_function(climate_modeling_month, sensor = "T3", variable = "range", class_name = "Near-Surface 째C")
